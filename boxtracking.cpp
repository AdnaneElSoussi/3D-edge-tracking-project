...
/// This function calculates edge direction and encodes it as a color ///
void Calaculate_dir_and_spread(Mat spreaded_dir, Mat frame);
...
////// reading camera input //////////
    ...
    //////////////////////////////////////
    int number_of_particles=300; /// Number of particles
...
/// INFINITE LOOP TO RECUSIVELY ESTIMATE CAMERA'S STATE ///
    for (film_count=0; film_count>-1; film_count++)
    {

        tstart = clock(); /// This is used to calculate the time per frame
        ///  initialize states vector ///
        if (film_count==0)
        {
            for(intialize_counter=0; intialize_counter<number_of_particles; intialize_counter++)
            {
                /// Initialize camera look at point///
                ...
                /// Initialize camera's roll angle ///
        ...
        else
        {
            for(intialize_counter=0; intialize_counter<number_of_particles; intialize_counter++)
            {
                ...
                /// lenght_of_vect calculates the distance between the camera's position and look at point
                ...
                /// perturb camera look at point proportional to the lenght_of_vect///
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 0) = (float)(States.at<float>(intialize_counter, 0)+my_rand*lenght_of_vect*0.05);
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 1) = (float)(States.at<float>(intialize_counter, 1)+my_rand*lenght_of_vect*0.05);
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 2) = (float)(States.at<float>(intialize_counter, 2)+my_rand*lenght_of_vect*0.05);

                /// perturb camera's roll angle ///
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 8) = (float)(States.at<float>(intialize_counter, 8)+my_rand);

                /// perturb camera position ///
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 3) = (float)(States.at<float>(intialize_counter, 3)+my_rand*2);
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 4) = (float)(States.at<float>(intialize_counter, 4)+my_rand*2);
                my_rand=(rand()%10000)/10000.0-0.5;
                States.at<float>(intialize_counter, 5) = (float)(States.at<float>(intialize_counter, 5)+my_rand*2);
            }
        ...
        if (film_count>0) /// Since we already did these step for the first frame, we skip these steps for the first frame.
        {
            cap >> frame; /// acquire frame
            resize(frame, frame, Size(320,240), 0, 0, INTER_LINEAR ); /// resize frame
            to_display=frame.clone(); /// To display estimate and real video
            to_display2=to_display2c.clone(); /// To display particle projections
            /// Clear matrices ///
            edges=cleared.clone();
        ...
        Calaculate_edges(frame, edges,12); /// Apply sobel filter and find edges
        dilate(edges, edges, Mat(), Point(-1, -1), 1, 1, 1); /// Spread edge pixels "This step is not neccessary"
        normalizeLetter(edges, edges); /// Thin edges to a single pixel
        Calaculate_dir_and_spread(direction, edges); /// Color encode edges accoring to their direction
        dilate(direction, direction, Mat(), Point(-1, -1), 2, 1, 1); /// Spread edge pixels

/// DRAW PARTICLE PROJECTIONS ON  "to_display2" ///

        for(intialize_counter=0; intialize_counter<number_of_particles; intialize_counter++)
        {
            /// Calculating the homogenous transformation from object frame of reference to camera frame of refrence saves it in "rotmat"
            ...
            /// inserts the camera intrinsic matrix in to "IntrnMat"
            IntrnMatCreate( IntrnMat);
            /// Transforms vertices from object frame of reference to camera frame of reference and saves them in "new vertices"
            tocameraspace( vertices, rotmat, newvertices);
            /// Finds the pixel coordinates of the "new vertices"
            topixels(newvertices,pixelcoords,IntrnMat);
            /// Normalizes weights such that the maximum weight is equal to one for display puposes
...
///     Likelyhood function     ///
        for(intialize_counter=0; intialize_counter<number_of_particles; intialize_counter++)
        {
            ...
            /// Finds the pixel coordinates of the "new vertices
            topixels(newvertices,pixelcoords,IntrnMat);
            ///  Calculates particle likelihood  /////////////////////////
            ...
            /// Saves likelihood. "Likelihoods are used to check if convergence of particles was correct"///
...
/// Normalize Weights ///
       ...
       /// Sorting ///
       /// get weights column and assign it to "one"
        one = States.col(6);
        /// Sort weights and acquire indices in "idx"
        cv::sortIdx(one, idx, cv::SORT_EVERY_COLUMN + cv::SORT_DESCENDING);
        /// Sort particles according weight in "New_States"
        for(int y = 0; y < States.rows; y++)
        {
            States.row(idx(0,y)).copyTo(New_States.row(y));
        }
        /// Replace old "States" with the sorted matrix
...
/// DRAW AVG///
        ...
        tend = clock();
        timer=(tend- tstart);
        // cout<< New_States.row(number_of_particles-1) <<endl;
...
void euler2ext(float A, float B, float C ,float X, float Y, float Z,Mat rotmat,float theta)
{
    ...
    yc.at<float>(0,2)=(float) ( (D-zc.at<float>(0,0)*yc.at<float>(0,0)-zc.at<float>(0,1)*yc.at<float>(0,1))/(zc.at<float>(0,2)) )*problem;
    yc=yc-pos;
    ...
    for( i=0; i<numberofedges; i++)
    {
        ...
        while (t<=1)
        {
            ...
            if (xn>=0 && xn<height && yn>=0 && yn<width)
            {
                ...
                if (difference<20 && (float)frame.at<uchar>(xn, yn)!=0  )
                {
                    counter=counter+1;
...
void Calaculate_dir_and_spread(Mat spreaded_dir, Mat frame)
{
    ...
    int i, j;
    float currentcolorx,currentcolory,currentcolor;
    float sidex1,sidey1,side1;
    float sidex2,sidey2,side2;
    float sidex3,sidey3,side3;
    float sidex4,sidey4,side4;
    float x1,x2,y1,y2;
    int sizze=6;
    int cc=0;
    int sett=0;
    for (i = (sizze); i<(height - (sizze)); i++)
    {
        for (j = (sizze); j<(width - (sizze)); j++)
        {

            if (frame.at<float>(i   , j   )==1)
            {
                cc=0;
                sett=0;
                sidex1=frame.at<float>(i +cc, j +sizze)*(i+cc);
                sidey1=frame.at<float>(i +cc, j +sizze);
                side1=frame.at<float>(i +cc, j +sizze);

                sidex2=frame.at<float>(i +cc, j -sizze)*(i+cc);
                sidey2=frame.at<float>(i +cc, j -sizze);
                side2=frame.at<float>(i +cc, j -sizze);

                sidex3=frame.at<float>(i +sizze, j );
                sidey3=frame.at<float>(i +sizze, j )*(j+cc);
                side3=frame.at<float>(i +sizze, j);

                sidex4=frame.at<float>(i +sizze, j );
                sidey4=frame.at<float>(i +sizze, j )*(j+cc);
                side4=frame.at<float>(i +sizze, j);
                ...
                while(cc<=(sizze))
                {
                    ...
                    sidex1=frame.at<float>(i +cc, j +sizze)*(i+cc)+frame.at<float>(i -cc, j +sizze)*(i-cc)+sidex1;
                    sidey1=frame.at<float>(i +cc, j +sizze)+frame.at<float>(i -cc, j +sizze)+sidey1;
                    side1=frame.at<float>(i +cc, j +sizze)+frame.at<float>(i -cc, j +sizze)+side1;



                    sidex2=frame.at<float>(i +cc, j -sizze)*(i+cc)+frame.at<float>(i -cc, j -sizze)*(i-cc)+sidex2;
                    sidey2=frame.at<float>(i +cc, j -sizze)+frame.at<float>(i -cc, j -sizze)+sidey2;
                    side2=frame.at<float>(i +cc, j -sizze)+frame.at<float>(i -cc, j -sizze)+side2;



                    sidex3=frame.at<float> (i +sizze, j +cc)+frame.at<float>       (i +sizze, j -cc)+sidex3;
                    sidey3=frame.at<float> (i +sizze, j +cc)*(j+cc)+frame.at<float>       (i +sizze, j -cc)*(j-cc)+sidey3;
                    side3 =frame.at<float> (i +sizze, j +cc)+frame.at<float>       (i +sizze, j -cc)+side3;



                    sidex4=frame.at<float> (i -sizze, j +cc)+frame.at<float>       (i -sizze, j -cc)+sidex4;
                    sidey4=frame.at<float> (i -sizze, j +cc)*(j+cc)+frame.at<float>(i -sizze, j -cc)*(j-cc)+sidey4;
                    side4 =frame.at<float> (i -sizze, j +cc)+frame.at<float>       (i -sizze, j -cc)+side4;
                ...
                sidey1=sidey1*(j +sizze)/side1;
                sidex1=sidex1/side1;

                sidey2=sidey2*(j -sizze)/side2;
                sidex2=sidex2/side2;

                sidey3=sidey3/side3;
                sidex3=sidex3*(i +sizze)/side3;

                sidey4=sidey4/side4;
                sidex4=sidex4*(i -sizze)/side4;
                ...
                if (sett==1)
                {
                    currentcolor= (atan((y2-y1)/(x2-x1))+M_PI/2)/M_PI*254+1;
                    if ((x2-x1)==0 && (y2-y1)!=0)
                    {
                        currentcolor=(M_PI/2+M_PI/2)/M_PI*254+1;
                    }

                    spreaded_dir.at<uchar>(i, j)=  (uchar)(currentcolor);
                }
...
void draw(Mat image,Mat pixelcoords, float score, int sides[][2])
{
    ...
    for( i=0; i<numberofedges; i++)
    {
        ...
        if (score>0)
        {
            line(image, pt1, pt2, Scalar(0,255*score,0), (1*score/10), CV_AA,0);
        }
        if (score==-1)
        {
